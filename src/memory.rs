...
pub fn register(meter: &Meter) -> Result<Box<(dyn CallbackRegistration)>, MetricsError> {
    let phys_mem: ObservableGauge<_> = meter.instrument("process.physical_memory.bytes").init();
    let virt_mem: ObservableGauge<_> = meter.instrument("process.virtual_memory.bytes").init();

    meter.register_callback(&[phys_mem.as_any(), virt_mem.as_any()], move |cb| {
